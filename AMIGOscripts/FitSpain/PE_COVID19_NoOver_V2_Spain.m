...
function [out] = PE_COVID19_NoOver_V2_Spain(epccOutputResultFileNameBase,epcc_exps,global_theta_guess,expdata,theta)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for iexp=1:length(Dat.Data.exp_data)
        ...
        mitigations{1,1}.val = 40; mitigations{1,1}.tmin = Dat.Data.start_date{iexp}; mitigations{1,1}.tmax = '12-Mar-2020';
%         mitigations{1,2}.val = 60; mitigations{1,2}.tmin = '1-mar-2020'; mitigations{1,2}.tmax = Dat.Data.end_date{iexp};
        try
            [cp, M_Tx, ~, ~] = Inputs_SIR(Dat.Data.start_date{iexp},'12-Mar-2020',0,mitigations);
            M_Ty = min(max(1-ExtractMitigation_NoInfected(Dat.Data.country_id{iexp}, Dat.Data.start_date{iexp},'12-Mar-2020'),0.01),1);
        catch
...
%         exp_indexData = exps_indexTraining(iexp);
        ...
        exps.obs_names{iexp} = char('Cum2Hospitalised',...
                                    'Cum2Critical    ',...
                                    'Cum2Recovered   ',...
                                    'Cum2Dead        ');
        exps.obs{iexp} = GetObser_V2_NoInfected_Spain(exps.obs_names{iexp});% Name of the observables 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        exps.t_f{iexp} = length(M_Tx)-1; 
        exps.n_s{iexp} = length(M_Tx);
        exps.t_s{iexp} = Dat.Data.t_s{iexp}(1,1:length(M_Tx));
...
%         end
        ...
        exps.exp_data{iexp} = Dat.Data.exp_data{iexp}(2:end,1:length(M_Tx))';
        exps.error_data{iexp} = Dat.Data.error_data{iexp}(2:end,1:length(M_Tx))';
    ...
    for i=1:inputs.exps.n_exp
         inputs.PEsol.id_local_theta_y0{i}=char('Exp1_0','Exp2_0','Exp3_0','Inf_0','Rec_0', ...
        ...
        botty0 = zeros(1,length(inputs.PEsol.id_local_theta_y0{i}));        
        ...
        for j=1:length(people)
            inity0(r(j):r(j)+(length(inputs.PEsol.id_local_theta_y0{i})/9-1)) = [repelem(round(people(j)-(people(j)*0.85)), (length(inputs.PEsol.id_local_theta_y0{1})/9))];
            botty0(r(j):r(j)+(length(inputs.PEsol.id_local_theta_y0{i})/9-1)) = [repelem(0, (length(inputs.PEsol.id_local_theta_y0{1})/9))];
