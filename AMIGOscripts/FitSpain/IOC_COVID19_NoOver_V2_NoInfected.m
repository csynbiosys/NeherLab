...
function [out] = IOC_COVID19_NoOver_V2_NoInfected(epccOutputResultFileNameBase,epcc_exps,global_theta_guess,expdata,theta)
    ...
    %% Load Model
    model = COVID19_NeherModel_V4_NoOver2_NoInfected;
    inputs.model = model;
...
%         end
        ...
        global_theta_guess(2) = 1;
        
        %% Compute Y0 (might not be required if we fit it, but we still need an initial guess)
            %%%%%%%%%%%%%%%%%%% INITIAL GUESS
        y0 = ComputeY0_COVID19_NoOver_DataBased_V2_NoInfected(AgeDistributions(Dat.Data.country_id{iexp}),...
            sum(Dat.Data.exp_data{iexp}(:,1)),...
            sum(AgeDistributions(Dat.Data.country_id{iexp})),...
            Dat.Data.exp_data{1});
    ...
    for iexp=1:inputs.exps.n_exp
     inputs.iocsol.n_steps{iexp}=Dat.Data.n_s{iexp}(1);
     inputs.iocsol.u_min{iexp}=[zeros(1,Dat.Data.n_s{iexp}(1)); cp];
     inputs.iocsol.u_max{iexp}=[ones(1,Dat.Data.n_s{iexp}(1)); cp];           % Minimum and maximum value for the input
     inputs.iocsol.u_guess{iexp}=[M_Ty;cp];       % Guess value for the input
     inputs.iocsol.t_con{iexp}=  M_Tx;
    end
    ...
    inputs.iocsol.id_par=model.par_names(param_including_vector,:);
    inputs.iocsol.par_guess=transpose(best_global_theta(param_including_vector));
    inputs.iocsol.par_max=global_theta_max(param_including_vector);  % Maximum allowed values for the parameters
    inputs.iocsol.par_min=global_theta_min(param_including_vector);  % Minimum allowed values for the parameters
    ...
    for i=1:inputs.exps.n_exp
         inputs.iocsol.id_y0{i}=char('Exp1_0','Exp2_0','Exp3_0','Inf_0','Rec_0', ...
                    'Exp1_1','Exp2_1','Exp3_1','Inf_1','Rec_1', ...
                    'Exp1_2','Exp2_2','Exp3_2','Inf_2','Rec_2', ...
                    'Exp1_3','Exp2_3','Exp3_3','Inf_3','Rec_3', ...
                    'Exp1_4','Exp2_4','Exp3_4','Inf_4','Rec_4', ...
                    'Exp1_5','Exp2_5','Exp3_5','Inf_5','Rec_5', ...
                    'Exp1_6','Exp2_6','Exp3_6','Inf_6','Rec_6', ...
                    'Exp1_7','Exp2_7','Exp3_7','Inf_7','Rec_7', ...
                    'Exp1_8','Exp2_8','Exp3_8','Inf_8','Rec_8');             % [] 'all'|User selected| 'none' (default)
     
        
        y0guess = ComputeY0Guess_COVID19_NoOver_DataBased_NoInfected(AgeDistributions(Dat.Data.country_id{iexp}),...
            sum(Dat.Data.exp_data{iexp}(:,1)),...
            sum(AgeDistributions(Dat.Data.country_id{iexp})),...
            Dat.Data.exp_data{1});
    
        people = AgeDistributions(Dat.Data.country_id{i});
        inity0 = zeros(1,length(inputs.iocsol.id_y0{i}));
        botty0 = zeros(1,length(inputs.iocsol.id_y0{i}));        
        
        r = 1:(length(inputs.iocsol.id_y0{i})/9):length(inputs.iocsol.id_y0{i});
        r2 = 1:length(exps.exp_y0{1})/9:length(exps.exp_y0{1});
        for j=1:length(people)
            inity0(r(j):r(j)+(length(inputs.iocsol.id_y0{i})/9-1)) = [repelem(round(people(j)-(people(j)*0.85)), (length(inputs.iocsol.id_y0{1})/9))];
            botty0(r(j):r(j)+(length(inputs.iocsol.id_y0{i})/9-1)) = [repelem(0, (length(inputs.iocsol.id_y0{1})/9))];
            botty0(r(j)+3) = exps.exp_y0{1}(r2(j)+4);
            botty0(r(j)+4) = exps.exp_y0{1}(r2(j)+8);
        end       
        if sum(inity0<y0guess) == 0
            inputs.iocsol.y0_max=inity0;                % Maximum allowed values for the initial conditions
            inputs.iocsol.y0_min=botty0;
        else
            inity0 = zeros(1,length(inputs.iocsol.id_y0{i}));
            r = 1:11:length(inputs.iocsol.id_y0{i});
            for j=1:length(people)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MODIFY
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% IF
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% WE
                ...
                inity0(r(j):r(j)+(length(inputs.iocsol.id_y0{i})/9-1)) = [people2(j),repelem(sum(Dat.Data.exp_data{1}(:,1)), length(inputs.iocsol.id_y0{i})/9-1)];
            end       
        end
%         inputs.PEsol.local_theta_y0_min{i}=repelem(0, length(inputs.PEsol.id_local_theta_y0{i}));                % Minimum allowed values for the initial conditions
        ...
        inputs.iocsol.id_y0=inputs.iocsol.id_y0{i};
        inputs.iocsol.y0_guess=y0guess;              % [] Initial guess
    end
...
%         inputs.PEsol.global_theta_y0_guess{i}=inputs.exps.exp_y0{i}(1:108);              % [] Initial guess
    ...
    inputs.iocsol.lsq_type='Q_expmax';
    ...
    inputs.nlpsol.eSS.maxtime = 50000;
...
%     inputs.rid.conf_ntrials=500;
    ...
    pe_start = now;
    pe_inputs = inputs;
    results = AMIGO_IOC(inputs);
    pe_results = results;
    pe_end = now;
    
    % Save the best theta
    best_global_theta(param_including_vector) = results.ioc.upar(1:length(find(param_including_vector==1)));
