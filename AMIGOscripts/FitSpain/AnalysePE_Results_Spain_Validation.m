...
ftag = ['PE_Test7_WebApp_R0epp_SpainData-'];
...
for j=1:100
    ...
    try
        ...
        for iexp=1:length(Dat.Data.exp_data)
            ...
            mitigations{1,1}.val = 40; mitigations{1,1}.tmin = Dat.Data.start_date{iexp}; mitigations{1,1}.tmax = '17-Apr-2020';
    %         mitigations{1,2}.val = 60; mitigations{1,2}.tmin = '1-mar-2020'; mitigations{1,2}.tmax = Dat.Data.end_date{iexp};

            try
                [cp, M_Tx, ~, ~] = Inputs_SIR(Dat.Data.start_date{iexp},'17-Apr-2020',0,mitigations);
                M_Ty = min(max(1-ExtractMitigation_NoInfected(Dat.Data.country_id{iexp}, Dat.Data.start_date{iexp},'17-Apr-2020'),0.01),1);
...
%                 M_Ty = 1-(0.5 + ((M_Ty-0)*(1-0.5))/(1-0));
                ...
                [cp, M_Tx, M_Ty, ~] = Inputs_SIR(Dat.Data.start_date{iexp},'17-Apr-2020',0,mitigations);
            end
    ...
    %         end
        ...
        lsf = '7-Apr-2020'; % The date is not inclusive
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ADD THE NORMALISATION BY THE EXPERIMENTAL MAXIMUM FOR NO VIAS????? 
        ...
        for iexp=1:length(Dat.Data.exp_data)
            ...
            if sum(sum(isnan(sim_results.sim.sim_data{iexp}((trd+1:end),:))))~=0
                nanpres(1,iexp) = 1;
            end
            SSE(iexp,:) = sum(((Dat.Data.exp_data{1,iexp}(2:end,(trd+1:end))'-sim_results.sim.sim_data{iexp}((trd+1:end),:)).^2)./max(Dat.Data.exp_data{1,iexp}(2:end,(trd+1:end))'),'omitnan');
        end

        sim_inputs = inputs;
        sim_exps = exps;
        save([strcat(dire,'\SimRun2-',num2str(j),'.mat')],'sim_results','sim_inputs','sim_exps','best_global_theta','SSE','nanpres');
...
for i=1:100
   try
       x = load([dire,'\SimRun2-',num2str(i),'.mat']);
       cfvs(i) = sum(x.SSE);
   catch
   end
end
...
bestsim = load([dire,'\SimRun2-',num2str(bestind),'.mat']);
