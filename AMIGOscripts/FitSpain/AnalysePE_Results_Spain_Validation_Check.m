...
for j=1:100
    ...
    try
        ...
        for iexp=1:length(Dat.Data.exp_data)
            ...
            mitigations{1,1}.val = 40; mitigations{1,1}.tmin = Dat.Data.start_date{iexp}; mitigations{1,1}.tmax = '7-Apr-2020';
    %         mitigations{1,2}.val = 60; mitigations{1,2}.tmin = '1-mar-2020'; mitigations{1,2}.tmax = Dat.Data.end_date{iexp};

            try
                [cp, M_Tx, ~, ~] = Inputs_SIR(Dat.Data.start_date{iexp},'7-Apr-2020',0,mitigations);
                M_Ty = min(max(1-ExtractMitigation_NoInfected(Dat.Data.country_id{iexp}, Dat.Data.start_date{iexp},'7-Apr-2020'),0.01),1);
...
%                 M_Ty = 1-(0.5 + ((M_Ty-0)*(1-0.5))/(1-0));
                ...
                [cp, M_Tx, M_Ty, ~] = Inputs_SIR(Dat.Data.start_date{iexp},'7-Apr-2020',0,mitigations);
            end
...
%             SSE(iexp,:) = sum(((Dat.Data.exp_data{1,iexp}(2:end,1:length(M_Tx))'-sim_results.sim.sim_data{iexp}(:,:)).^2)./max(Dat.Data.exp_data{1,iexp}(2:end,1:length(M_Tx))'),'omitnan');
            
            divs = ones(size(sim_results.sim.sim_data{iexp}(:,:))).*(1./max(Dat.Data.exp_data{1,iexp}(2:end,1:length(M_Tx))'));
            
            SSE(iexp,:) = sum(((Dat.Data.exp_data{1,iexp}(2:end,1:length(M_Tx))'-sim_results.sim.sim_data{iexp}(:,:)).^2).*divs,'omitnan');
...
subplot(5,1,1)
hold on
plot(Dat.Data.t_s{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))), Dat.Data.exp_data{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))))
...
subplot(5,1,2)
hold on
plot(Dat.Data.t_s{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))), Dat.Data.exp_data{1}(2,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))))
% plot(bestfit.pe_results.sim.tsim{1}, bestfit.pe_results.sim.obs{1}(:,1))
plot(bestsim.sim_results.sim.tsim{1}, cumResCov19(:,11))
...
subplot(5,1,3)
hold on
plot(Dat.Data.t_s{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))), Dat.Data.exp_data{1}(3,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))))
% plot(bestfit.pe_results.sim.tsim{1}, bestfit.pe_results.sim.obs{1}(:,2))
plot(bestsim.sim_results.sim.tsim{1}, cumResCov19(:,12))
title('Cri')
% set(gca, 'YScale', 'log')
...
subplot(5,1,4)
hold on
plot(Dat.Data.t_s{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))), Dat.Data.exp_data{1}(4,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))))
...
subplot(5,1,5)
hold on
plot(Dat.Data.t_s{1}(1,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))), Dat.Data.exp_data{1}(5,1:length(bestfit.pe_results.sim.exp_data{1}(:,1))))
