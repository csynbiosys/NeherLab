...
%         exp_indexData = exps_indexTraining(iexp);
        ...
        exps.u{iexp} = [M_Ty(1:trd);cp(1:trd)];
        exps.t_con{iexp} = M_Tx(1:trd);
        
        exps.t_f{iexp} = trd-1; 
        exps.n_s{iexp} = trd;
        exps.t_s{iexp} = Dat.Data.t_s{iexp}(1,(1:trd));
...
%         end
        ...
        exps.exp_data{iexp} = Dat.Data.exp_data{iexp}(:,(1:trd))';
        exps.error_data{iexp} = Dat.Data.error_data{iexp}(:,(1:trd))';
...
%         inputs.PEsol.global_theta_y0_guess{i}=inputs.exps.exp_y0{i}(1:108);              % [] Initial guess
    ...
    inputs.nlpsol.eSS.log_var = [1:length(find(param_including_vector==1))];%[find(param_including_vector==1)]; % Modify this
%     according to the parameters we want to fit                       
