function [results, inputs, privstruct]=AMIGO_PE_UpdateY0(input_file,run_ident,opt_solver,opts_solver)
...
[inputs]=AMIGO_check_model_NoVer(input_file,inputs_def);


[inputs]=AMIGO_check_exps_NoVer(inputs);
...
[results,privstruct]=AMIGO_call_OPTsolver_UpdateY0(...
    'PE',...
...
% if ~strcmpi(inputs.model.exe_type, 'fullMex')
    ...
    try
    [results.fit.f,results.fit.h,results.fit.R] = AMIGO_PEcost_UpdateY0(...
        results.nlpsol.vbest,...
        inputs,...
        results,...
        privstruct);
...
if inputs.nlpsol.regularization.ison
    [f,~,~,regobj,~] = AMIGO_PEcost_UpdateY0(results.nlpsol.vbest,inputs,results,privstruct);
...
%% Added By David
[privstruct,inputs]=AMIGO_transform_Y0_TS(inputs,results,privstruct);
%%
