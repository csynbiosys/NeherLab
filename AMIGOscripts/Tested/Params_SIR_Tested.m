...
function [par] = Params_SIR_Tested(N,R0,epp,ofs,t_i,t_l,t_c,t_h,report,HB,ICUb,imports,confirmed,severe,critical,fatality,isolated)
...
%     fatality = ([0.002, 0.006, 0.03, 0.08, 0.15, 0.6, 2.2, 5.1, 9.3]) / 100;
    ...
    if isempty(t_i); t_i = 3; end

    if isempty(t_l); t_l = 3; end

    if isempty(t_c); t_c = 14; end

    if isempty(t_h); t_h = 3; end
    
    if isempty(report); report = 0.3; end
...
%     if isempty(fatality); fatality = ([0.002, 0.006, 0.03, 0.08, 0.15, 0.6, 2.2, 5.1, 9.3]) / 100; else; fatality  = (fatality) / 100; end
...
par=[N,R0,epp,ipd,ofs,HB,ICUb,t_i,t_l,t_c,t_h,report,...
