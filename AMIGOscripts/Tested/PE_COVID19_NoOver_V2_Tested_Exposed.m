...
function [out] = PE_COVID19_NoOver_V2_Tested_Exposed(epccOutputResultFileNameBase,epcc_exps,global_theta_guess,expdata,theta)
    ...
    %% Load Model
    model = COVID19_NeherModel_V4_NoOver2_Tested;
    inputs.model = model;
...
%         mitigations{1,2}.val = 60; mitigations{1,2}.tmin = '1-mar-2020'; mitigations{1,2}.tmax = Dat.Data.end_date{iexp};
        try
            ...
            M_Ty = min(max(1-ExtractMitigation_Tested(Dat.Data.country_id{iexp}, Dat.Data.start_date{iexp},Dat.Data.end_date{iexp}),0.01),1);
        catch
...
%         exp_indexData = exps_indexTraining(iexp);
        ...
        exps.obs{iexp} = GetObser_V2_Tested(Dat.Data.obs{iexp});% Name of the observables 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         end
        ...
        %% Compute Y0 (might not be required if we fit it, but we still need an initial guess)
            %%%%%%%%%%%%%%%%%%% INITIAL GUESS
        y0 = ComputeY0_COVID19_NoOver_DataBased_V2_Tested_Exposed(AgeDistributions(Dat.Data.country_id{iexp}),sum(Dat.Data.exp_data{iexp}(:,1)),...
    ...
    for i=1:inputs.exps.n_exp
         inputs.PEsol.id_local_theta_y0{i}=char('Sus_0', ...
                    'Sus_1', ...
                    'Sus_2', ...
                    'Sus_3', ...
                    'Sus_4', ...
                    ...
                    'Sus_6', ...
                    'Sus_7', ...
                    'Sus_8');             % [] 'all'|User selected| 'none' (default)
     
        
        y0guess = ComputeY0Guess_COVID19_NoOver_DataBased_Tested_Exposed(AgeDistributions(Dat.Data.country_id{iexp}),...
        ...
        ages = (AgeDistributions(Dat.Data.country_id{iexp})) / (sum(AgeDistributions(Dat.Data.country_id{iexp})));
        
        r = 1:(length(inputs.PEsol.id_local_theta_y0{i})/9):length(inputs.PEsol.id_local_theta_y0{i});
        for j=1:length(people)
            inity0(r(j):r(j)+(length(inputs.PEsol.id_local_theta_y0{i})/9-1)) = [people(j)-(sum(Dat.Data.exp_data{iexp}(:,1)).*ages(j))];
            botty0(r(j):r(j)+(length(inputs.PEsol.id_local_theta_y0{i})/9-1)) = [people(j)-(people(j)*0.3)];
        end       
