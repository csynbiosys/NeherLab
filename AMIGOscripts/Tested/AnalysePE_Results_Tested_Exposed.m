...
ftag = ['PE_Test1_HalfY0_TestedAndTs-'];
...
for i=1:9
    bestfit.pe_inputs.exps.exp_y0{1}(r(i)) = bestfit.pe_results.fit.local_theta_y0_estimated{1}((i));
...
r = 1:12:length(bestfit.pe_inputs.exps.exp_y0{1});
r2 = 5:12:length(bestfit.pe_inputs.exps.exp_y0{1});
for i=1:length(r)
    sus = bestfit.pe_inputs.exps.exp_y0{1}(r(i));
    tots = AgeDistributions('ITA');
    infe = (tots((i))-sus);
    bads = (bestfit.pe_inputs.exps.exp_y0{1}(r2(i):r2(i)+4));
    bads(2) = [];
    infe = infe+sum(bads);
    bestfit.pe_inputs.exps.exp_y0{1}(r(i)+1:r(i)+3) = repelem(infe/3, 3);
