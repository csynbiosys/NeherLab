...
for iexp=1:inputs.exps.n_exp
    ...
    r2 = 5:12:length(inputs.exps.exp_y0{1});
    for i=1:length(r)
        sus = privstruct.y_0{iexp}(r(i));
        tots = AgeDistributions('ITA');
        infe = (tots((i))-sus);
        bads = (inputs.exps.exp_y0{iexp}(r2(i):r2(i)+4));
        bads(2) = [];
        infe = infe+sum(bads);
        privstruct.y_0{iexp}(r(i)+1:r(i)+3) = repelem(infe/3, 3);
        privstruct.exp_y0{iexp}(r(i)+1:r(i)+3) = repelem(infe/3, 3);
        inputs.exps.exp_y0{iexp}(r(i)+1:r(i)+3) = repelem(infe/3, 3);
